...
def make_prediction(tick="AMZN", days=1):
    ...
    future_price = str(round(predict(model, data), 2))
    
    # Now we will calculate the accuracy by counting the number of positive profits
    accuracy_score = round((len(final_df[final_df['sell_profit'] > 0]) + len(final_df[final_df['buy_profit'] > 0])) / len(final_df), 2)
