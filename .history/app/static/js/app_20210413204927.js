...
function showData(subject) {
    d3.json("data/stocks.json").then((importedData) => {
        ...
        importedData.ticks.forEach((value) => {
            ...
            let ticker = e.options[e.selectedIndex].value;
            // document.getElementById("result").innerHTML = ticker;
        });
    });
}
...
// Capturing stock info based on drop-down choice
function stockData(subject) {
    let stockInfo = yf.Ticker(ticker)
    let name = stockInfo.info["longName"]
    console.log(name)
    return name;
}
